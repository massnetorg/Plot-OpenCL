...
public:
    Ploter() {
        ...
        cl_output_obj = clCreateBuffer(cl_ctx, CL_MEM_WRITE_ONLY, MAX_TABLE_A_INPUT_SIZE * sizeof(unsigned int), NULL, &cl_ret);
        FILE *fp;
        char *source_str;
        size_t source_size;
        ...
        cl_krnl = clCreateKernel(cl_prg, "sha256_mass_table_a", &cl_ret);
        cl_ret = clSetKernelArg(cl_krnl, 0, sizeof(cl_mem), (void *)&cl_input_obj);
        cl_ret = clSetKernelArg(cl_krnl, 1, sizeof(cl_mem), (void *)&cl_pubkey_hash_obj);
        cl_ret = clSetKernelArg(cl_krnl, 2, sizeof(cl_mem), (void *)&cl_output_obj);
        global_item_size = MAX_TABLE_A_INPUT_SIZE;
        group_item_size = 64;
    ...
    ~Ploter() {
        ...
        cl_ret = clReleaseKernel(cl_krnl);
    ...
    void calc_table_A(unsigned int *pubkeyHash, unsigned int bl) {
        ...
        unsigned int *output = new unsigned int[max_block_size];
        ...
        for (int j = 0; j < block_nums; j++) {
            ...
            cl_ret = clEnqueueNDRangeKernel(cl_cmd_q, cl_krnl, 1, NULL,
            ...
            cl_ret = clEnqueueReadBuffer(cl_cmd_q, cl_output_obj, CL_TRUE, 0,
                                         MAX_TABLE_A_INPUT_SIZE * sizeof(unsigned int), output, 0, NULL, NULL);
            end = std::chrono::system_clock::now();
            ...
            store_table_a_duration += duration_cast<std::chrono::microseconds>(end - start);
            for(int i = 0; i < 16; i++) {
                printf("sha256(%u) = %u\n", i, output[i]);
            }
        }
        ...
        std::cout << "copy_buffser: " << double(copy_buffer_duration.count()) * std::chrono::microseconds::period::num / std::chrono::microseconds::period::den << "s" << std::endl;
        std::cout << "store_A: " << double(store_table_a_duration.count()) * std::chrono::microseconds::period::num / std::chrono::microseconds::period::den << "s" << std::endl;
